...
import QQAccountSetupModal from './personal/modals/QQAccountSetupModal';
import './personal/personal-glass.css';
...
const PersonalSetting = () => {
  ...
  const accountSetupCompletedRef = useRef(false);
  ...
  const setupAccountRequested = searchParams.get('setup_account') === '1';
  const bindSource = searchParams.get('bind_source');
  const shouldRequireAccountSetup =
    userState?.user?.require_account_setup === true;

  useEffect(() => {
    if (
      (setupAccountRequested || shouldRequireAccountSetup) &&
      userState?.user &&
      !accountSetupCompletedRef.current
    ) {
      setShowAccountSetupModal(true);
      setInputs((prev) => ({
        ...prev,
        account_username:
          prev.account_username || userState.user.username || '',
      }));
    }
  }, [
    setupAccountRequested,
  ...
  const bindEmail = async () => {
    ...
    if (success) {
      ...
      nextParams.delete('bind_email');
      nextParams.delete('bind_source');
  ...
  const completeQQAccountSetup = async () => {
    const username = normalizeUsername(inputs.account_username);
    const password = inputs.account_password || '';
    const confirmPassword = inputs.account_password_confirmation || '';
    ...
    try {
      ...
      showSuccess(t('\u8d26\u53f7\u8bbe\u7f6e\u5b8c\u6210'));
      accountSetupCompletedRef.current = true;
      setShowAccountSetupModal(false);
      const nextParams = new URLSearchParams(searchParams);
      nextParams.delete('setup_account');
      nextParams.delete('bind_source');
      setSearchParams(nextParams, { replace: true });
      setInputs((prev) => ({
        ...prev,
    ...
    <div className='ps-page'>
      <div className='flex justify-center'>
        <div className='w-full max-w-7xl mx-auto px-2'>
          ...
          <UserInfoHeader t={t} userState={userState} status={status} />
      ...
      <QQAccountSetupModal
        t={t}
        visible={showAccountSetupModal}
        setVisible={setShowAccountSetupModal}
        inputs={inputs}
        handleInputChange={handleInputChange}
        onSubmit={completeQQAccountSetup}
