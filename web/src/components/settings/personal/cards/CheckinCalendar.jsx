...
const CheckinCalendar = ({ t, status }) => {
  // Use site key from status directly, regardless of whether global turnstile_check is on
  ...
  // 是否需要 Turnstile 验证
  const turnstileRequired = checkinData.turnstile_required && !!turnstileSiteKey;
  ...
  const doCheckin = async (token) => {
    ...
    try {
      ...
      if (success) {
        ...
        if (token) {
          // Token was invalid — reset the widget so user can retry
  ...
  return (
    <Card className='ps-card !rounded-2xl' bodyStyle={{ background: 'transparent' }}>
      <Modal
        title={t('安全验证')}
