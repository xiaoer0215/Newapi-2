...
const SystemSetting = () => {
  ...
  const submitQQOAuth = async () => {
    ...
    if (originInputs['QQOAuthBaseUrl'] !== inputs.QQOAuthBaseUrl) {
      options.push({
        key: 'QQOAuthBaseUrl',
        value: removeTrailingSlash(inputs.QQOAuthBaseUrl || ''),
      });
    }
            ...
            >
              ...
              <Card>
                <Form.Section text={t('配置 QQ OAuth')}>
                  ...
                  >
                    {t(
                      '如果你使用的是彩虹聚合登录，请填写其接口地址。该服务可由任何人搭建，选择服务商时请注意可靠性，服务商停止服务会导致相关用户无法登录。',
                    )}
                  </Text>
                  <Banner
                    type='info'
                  ...
                  >
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input
                        field='QQClientId'
                        label={t('QQ Client ID')}
                      />
                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input
                        field='QQClientSecret'
                        label={t('QQ Client Secret')}
                        type='password'
                        placeholder={t('敏感信息不会发送到前端显示')}
                      />
                    </Col>
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Input
                        field='QQOAuthBaseUrl'
                        label={t('彩虹聚合登录接口地址')}
                        placeholder='https://connect.qq.com'
                        extraText={t(
                          '默认是 https://connect.qq.com，使用彩虹聚合登录时改成你的服务商接口地址',
                        )}
                      />
                    </Col>
                  </Row>
