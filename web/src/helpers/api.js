...
export let API = axios.create({
  ...
  headers: {
    ...
    'X-NewAPI-Device-Fingerprint': getDeviceFingerprint(),
  },
});
...
export async function onQQOAuthClicked(
  qq_client_id,
  qq_oauth_base_url = '',
  options = {},
) {
  void qq_oauth_base_url;
  const state = await prepareOAuthState(options);
  if (!state) return;
  if (!qq_client_id) {
    showError('QQ Client ID 未配置');
    return;
  }

  const startUrl = new URL('/api/oauth/qq/start', window.location.origin);
  startUrl.searchParams.set('state', state);
  redirectToOAuthUrl(startUrl);
}
