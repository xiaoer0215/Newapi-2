...
// -------------------------------
// 模型定价计算工具函数
const hasValidPriceValue = (value) =>
  value !== undefined &&
  value !== null &&
  value !== '' &&
  Number.isFinite(Number(value));

export const isPricingModelConfigured = (record) => {
  if (!record) return false;

  if (typeof record.pricing_configured === 'boolean') {
    return record.pricing_configured;
  }

  if (record.quota_type === 1) {
    return hasValidPriceValue(record.model_price);
  }

  if (record.quota_type === 0) {
    return hasValidPriceValue(record.model_ratio);
  }

  return (
    hasValidPriceValue(record.model_price) ||
    hasValidPriceValue(record.model_ratio)
  );
};
