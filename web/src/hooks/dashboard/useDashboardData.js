...
export const useDashboardData = (userState, userDispatch, statusState) => {
  ...
  const contactEnabled = statusState?.status?.contact_enabled ?? false;
  const contact2Enabled = statusState?.status?.contact2_enabled ?? false;

  const hasApiInfoPanel = apiInfoEnabled;
  const hasInfoPanels = announcementsEnabled || faqEnabled || uptimeEnabled || contactEnabled || contact2Enabled;
