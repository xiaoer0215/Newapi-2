...
export const useLogsData = () => {
  ...
  const handleEyeClick = async () => {
    ...
    try {
      ...
      // ignore — still show the stat bar with whatever data we have
    } finally {
      setShowStat(true);
  ...
  const removedModalHook = (log) => {
    if (!log || !log.content) {
      return;
    }
    const other = getLogOther(log.other);
    setRemovedModalTarget({
      title: Number(log.type) === 5 ? t('错误详情') : t('详情'),
      content: log.content,
      modelName: log.model_name || '',
      requestId: log.request_id || '',
