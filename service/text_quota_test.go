...
func TestNormalizeUsageUsesInputOutputAndInputDetails(t *testing.T) {
	...
	require.Equal(t, 40, usage.PromptTokensDetails.CachedCreationTokens)
	require.Equal(t, 1200, usage.PromptTokensDetails.TextTokens)
}

func TestCalculateTextQuotaSummaryUsesInputOutputTokensWhenPromptCompletionMissing(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	...
	require.Equal(t, 1130, summary.Quota)
}
