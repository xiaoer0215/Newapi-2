...
func TestGetPreferredChannelByAffinity_IncludeModelNameSeparatesCache(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	affinityValue := fmt.Sprintf("pc-model-%d", time.Now().UnixNano())
	rule := setting.Rules[0]
	cacheKeySuffix := buildChannelAffinityCacheKeySuffix(rule, "gpt-5", "default", affinityValue)

	cache := getChannelAffinityCache()
	require.NoError(t, cache.SetWithTTL(cacheKeySuffix, 10086, time.Minute))
	t.Cleanup(func() {
		_, _ = cache.DeleteMany([]string{cacheKeySuffix})
	})
	...
	channelID, found := GetPreferredChannelByAffinity(newContext(), "gpt-5", "default")
	require.True(t, found)
	require.Equal(t, 10086, channelID)

	channelID, found = GetPreferredChannelByAffinity(newContext(), "gpt-4o", "default")
