		...
		if claudeMessage.IsStringContent() {
			...
			for _, mediaMsg := range contents {
				...
				case "document":
					fileData := ""
					if mediaMsg.Source != nil {
						fileData = mediaMsg.Source.Url
						if fileData == "" {
							mediaType := mediaMsg.Source.MediaType
							if mediaType == "" {
								mediaType = "application/pdf"
							}
							fileData = fmt.Sprintf("data:%s;base64,%s", mediaType, common.Interface2String(mediaMsg.Source.Data))
						}
					}
					if fileData != "" {
