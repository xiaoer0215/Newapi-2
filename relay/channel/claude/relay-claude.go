					...
					if mediaMessage.Type != "text" {
						...
						if mediaMessage.Type == dto.ContentTypeFile {
							if file := mediaMessage.GetFile(); file != nil {
								...
								if ext != "" {
									fileMimeHint = service.GetMimeTypeByExtension(ext)
								}
								if source.IsBase64() && source.MimeType == "" && fileMimeHint != "" && fileMimeHint != "application/octet-stream" {
									source.MimeType = fileMimeHint
								}
							}
						}
						...
						if (mimeType == "" || mimeType == "application/octet-stream") && fileMimeHint != "" && fileMimeHint != "application/octet-stream" {
							mimeType = fileMimeHint
						}
						...
						if strings.HasPrefix(mimeType, "text/") {
							...
							claudeMediaMessages = append(claudeMediaMessages, dto.ClaudeMediaMessage{
								Type: "text",
								Text: common.GetPointer(string(decodedText)),
							})
							continue
						}
						...
						case mimeType == "" || strings.HasPrefix(mimeType, "image/"):
							claudeMediaMessage.Type = "image"
						default:
							return nil, fmt.Errorf("mime type is not supported: %s", mimeType)
						}
...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if claudeInfo.Usage.CompletionTokens == 0 || !claudeInfo.Done {
		...
		// Only supplement missing fields so usage captured earlier, such as cache
		// fields from message_start, is not lost when stream usage is incomplete.
