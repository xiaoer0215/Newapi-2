...
上流リポジトリからではなく、この二次開発版リポジトリを直接デプロイしたい場合は、次のリポジトリを利用してください。
...
> 補足：
> - 下記の公式リポジトリ、イメージ、ドキュメントへのリンクは、上流参照と互換性説明のために残しています
> - このフォークだけで配布を完結させたい場合は、自分の Releases、イメージ配布先、更新履歴、ドキュメントトップも合わせて整備することをおすすめします

### このリポジトリで整理している主なカスタム機能

- 監視強化：グループ監視とモデル監視を追加し、状態、成功率、失敗回数などをより詳しく確認可能
- フロント側と管理画面の UI 調整：ログイン登録、モデル広場、トークン画面などの見た目と操作を最適化
- モデル広場の出典説明：モデル広場ページは [one-hub](https://github.com/MartialBE/one-hub) を参考にした模倣実装です
- ユーザー向け AI 画像生成：サイト内フローと残高決済に対応した基本的な AI 画像生成機能を追加
- カスタム可能なデータダッシュボード：表示モジュールを必要に応じて構成でき、QQ グループやサポート連絡先向けの QR コード枠を 2 つ追加可能
- 個人向けレート制限：分、時間、日単位でユーザーごとのリクエスト上限を設定可能
- ウォレットとサブスクリプション管理：割引や付与クレジットを可視化し、サブスクリプション実例管理とユーザーごとの購読内容確認に対応
- 交換コードと自動配送：交換コードの検索時に関連ユーザーを連動表示でき、自動配送では商品公開、カード情報の取り込みと販売に対応
- ログとホームページ上書き：リクエストログ、エラーログ、消費ログに対応し、HTML によるホームページ上書き時にナビゲーションとフッターを完全に覆えなかった問題を修正。ただし `new-api` の著作権表記は保持が必要
- 請求検索の改善：ユーザー名または支払い番号で支払い記録を検索でき、旧来の注文番号表示ロジックを廃止
