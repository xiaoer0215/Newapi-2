...
func InitDB() (err error) {
	...
	if err == nil {
		...
		if common.UsingSQLite {
			// SQLite is a single-writer database; cap connections to prevent SQLITE_BUSY
...
func migrateDB() error {
	...
	// Migrate tables one by one, ignoring errors for tables that already exist
	models := []interface{}{
		&Channel{},
		&Token{},
		&User{},
		...
		&AutoDeliveryProduct{},
		&AutoDeliveryCard{},
		// AutoDeliveryOrder is handled separately: AutoMigrate hangs on SQLite when
		// the table already exists with an inline UNIQUE constraint on trade_no.
	}

	for _, model := range models {
		if err := DB.AutoMigrate(model); err != nil {
			// Log error but continue with other tables
			common.SysError("failed to migrate table: " + err.Error())
		}
	}

	// AutoDeliveryOrder: only AutoMigrate to create the table on fresh install.
	// If the table already exists, skip AutoMigrate to avoid SQLite hang, and let
	// addAutoDeliveryColumns() add any missing columns via ALTER TABLE instead.
	if !DB.Migrator().HasTable(&AutoDeliveryOrder{}) {
		if err := DB.AutoMigrate(&AutoDeliveryOrder{}); err != nil {
			common.SysError("failed to migrate auto_delivery_orders: " + err.Error())
		}
	}

	// Manually add new columns that AutoMigrate may fail to add for existing tables
	// (e.g. SQLite tables with UNIQUE inline constraints prevent GORM table-rebuild)
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&AutoDeliveryProduct{}, "AutoDeliveryProduct"},
		{&AutoDeliveryCard{}, "AutoDeliveryCard"},
		// AutoDeliveryOrder handled separately below
	}
	...
	// AutoDeliveryOrder: only AutoMigrate if table doesn't exist to avoid SQLite hang
	if !DB.Migrator().HasTable(&AutoDeliveryOrder{}) {
		if err := DB.AutoMigrate(&AutoDeliveryOrder{}); err != nil {
			common.SysError("failed to migrate auto_delivery_orders: " + err.Error())
		}
