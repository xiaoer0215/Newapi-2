...
如果你希望直接使用當前二開倉庫，而不是從上游倉庫開始，請優先使用下面的倉庫地址：
...
> 說明：
> - 當前文件中保留的官方倉庫、鏡像與文件連結，主要用於說明上游來源與相容關係
> - 如果你計畫完全透過當前倉庫分發，建議同步維護自己的 Releases、鏡像倉庫、更新日誌與文件首頁

### 當前倉庫已整理的二開與定制能力

- 監控增強：新增分組監控與模型監控，支援查看更多狀態、成功率、失敗次數等資訊
- 前台與控制台 UI 調整：登入註冊、模型廣場、令牌介面等頁面已做風格與互動優化
- 模型廣場來源說明：模型廣場頁面仿製 [one-hub](https://github.com/MartialBE/one-hub)
- 用戶端 AI 生圖：提供基礎 AI 生圖功能，支援站內生圖流程與餘額結算
- 數據看板定制：看板內容可按需展示，並可額外放置兩個二維碼位，適合 QQ 群、客服微信等場景
- 個人限速：支援按分鐘、小時、天為用戶設定請求限額
- 錢包與訂閱管理：錢包介面支援折扣、贈送額度等可視化展示；新增訂閱實例管理，可查看具體用戶訂閱內容
- 兌換碼與自動發貨：兌換碼支援搜尋後聯動用戶查詢；自動發貨支援商品上架、卡密導入與販售
- 日誌與首頁覆蓋：支援啟用請求日誌、錯誤日誌、額度消費日誌，並修復 HTML 覆蓋首頁時可完整覆蓋導覽列與底部的問題，但仍需保留 `new-api` 版權資訊
- 帳單檢索優化：支援按用戶名或支付號查詢支付記錄，並取消原版訂單號展示邏輯
