...
func (m *MediaContent) ToFileSource() *types.FileSource {
	switch m.Type {
	case ContentTypeImageURL:
		image := m.GetImageMedia()
		if image == nil || image.Url == "" {
			return nil
		}
		return createFileSource(image.Url)
	case ContentTypeInputAudio:
		audio := m.GetInputAudio()
		if audio == nil || audio.Data == "" {
			return nil
		}
		source := createFileSource(audio.Data)
		if source != nil && audio.Format != "" && source.IsBase64() && source.MimeType == "" {
			source.MimeType = "audio/" + audio.Format
		}
		return source
	case ContentTypeFile:
		file := m.GetFile()
		...
		return createFileSource(file.FileData)
	case ContentTypeVideoUrl:
		video := m.GetVideoUrl()
		if video == nil || video.Url == "" {
			return nil
		}
		return createFileSource(video.Url)
	default:
		return nil
	}
}
